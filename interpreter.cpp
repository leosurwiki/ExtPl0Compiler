...
void interpreter::interpreter::symbol_load(string s)
{
	...
	for (int i=functionstack.size();i>=0;i--)
	{
		if (s.find(get_trait_string(i))!=string::npos||i==-1)
		{
			...
			{
				if (flow::color.find(s)!=flow::color.end()&&flow::color[s]<8)
				{
					fout<<"add $t0,$s"<<flow::color[s]<<",$0"<<endl;break;
					interpreter::reg2mem[flow::color[s]]=string2shiftfromsp[s];
				}
				else
				{
					fout<<"lw $t0,-"<<string2shiftfromsp[s]<<"($sp)"<<endl;break;
				}
			}			
			else
			{
	...
	for (int i=functionstack.size();i>=0;i--)
	{
		if (s.find(get_trait_string(i))!=string::npos||i==-1)
		{
			if (i==functionstack.size())
			{
				if (flow::color.find(s)!=flow::color.end()&&flow::color[s]<8)
				{
					fout<<"add $s"<<flow::color[s]<<",$t8,$0"<<endl;	
					interpreter::reg2mem[flow::color[s]]=string2shiftfromsp[s];
				}
...
void interpreter::interpreter::interpreter_subprogram(quartral_element qe)
{
	...
	fout<<"addi $sp,$sp,-4"<<endl;
	for (map<int,int>::iterator idx=reg2mem.begin();idx!=reg2mem.end();idx++)
	{
		fout<<"lw $s"<<idx->first<<",-"<<idx->second<<"($sp)"<<endl;
	}
}
